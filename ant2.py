...
class Ant:
    ...
    def move(self, output, current_vision):
        ...
        if self.isOut(cur_x, cur_y):
            ...
            performance += self.perfomanceCalc(current_vision)
            if self.canEat(cur_x, cur_y):
                performance += 1000
                self.allowed_moves += 20
    ...
    def perfomanceCalc(self, current_vision):
        ...
        performance =  (cur_min_dist - self.prev_min_dist)  + 0.3 * (cur_avg_dist - self.prev_avg_dist)
    ...
    def createVision(self):
        ...
        if self.orientation == 0:
            # upside viewing
          for y in range(self.y-1, self.y-5, -1):
          ...
          # leftside viewing
          for x in range(self.x-1, self.x-5, -1):
            for y in range(self.y+2, self.y-3, -1):
              valid = self.validatePoint(x, y)
                    # tmp - [x, y, Food, Wall]
              tmp = list()
              if valid:
          ...
          # downside viewing
          for y in range(self.y+1, self.y+5):
            for x in range(self.x+2, self.x-3, -1):
              valid = self.validatePoint(x, y)
                    # tmp - [x, y, Food, Wall]
              tmp = list()
              if valid:
          ...
          # rightside viewing
          for x in range(self.x+1, self.x+5):
...
def main(genomes, config):
    ...
    while QUIT == False:
        ...
        #     else:
        #         ge[ant_id].fitness += performance + 0.5
